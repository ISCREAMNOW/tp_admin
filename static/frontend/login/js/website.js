...
// data.debug - Debug mode. The default value is false
    ...
    seajs.config = function(configData) {
        configData.base = '/static/frontend/login/js/';
...
seajs.on('fetch', function(data) {
    if(data && data.uri) {
        data.requestUri = data.uri.replace("{{0}}", seajs.Module.cdn % 6);
        seajs.Module.cdn++;
    }
...
(function (root, factory) {
    ...
    $.fn.lazyhover = function (fuc_on, de_on, de_out) {
        ...
        self.each(function (index, el) {
            var mall_dot = $(el);
            mall_dot.bind("mouseenter",function () {
                flag = 1;
                timer(mall_dot);
    ...
    return $.fn.initSlide = function (config) {
        var defaults = {fontsize: 12, right: 10, bottom: 15, time: 5000, autosize: 0, slidearrow: true, dot: true};
        return this.each(function () {
            var _this = $(this);
            _this.config = $.extend(defaults, config);
            _this.n = 0;
            _this.j = 0;
            _this.first_show = 1;
            var t;
            var _obj = _this.find(".slider_container");
            var list = _this.find(".slider li");
            _this.count = list.length;
            var factory = function () {
                //元素定位
                var slide_control = $("<div class='slide_control'></div>");
                $(_obj).prepend(slide_control);

                list.css({position: "absolute", top: "0", left: "0", width: "100%", height: "100%", overflow: "hidden"}).hide().each(function (i) {
                    slide_control.append("<a>" + (i + 1) + "</a>");
                });
                ...
                resetclass(_obj.find(".slide_control a"), 0);

//                //第一张图片lazyload
//                var img_li = list.first();
//                var f_img=$("img[lazyload]",img_li);
//                //商城第一帧图片不止一张所以这里要用数组
//                $.each(f_img,function(i){
//                    _this.attr("src",_this.attr("lazyload")).removeAttr("lazyload");
//                });
...
//                }
                ...
                t = setInterval(autoplay, _this.config.time);
                //force the first slide to show

                _obj.find(".slide_control a").eq(0).triggerHandler('mouseenter');

            };
            //加一个上一页下一页的按钮
            var slidearrow = function () {
                var prev_button, next_button;
                if (_this.config.slidearrow) {
                    prev_button = $('<em class="slidearrow slidearrow_l"></em>');
                    next_button = $('<em class="slidearrow slidearrow_r"></em>');
                    _obj.append(prev_button).append(next_button);
                } else {
                    var btns = _obj.find(".img_slider_btn");
                    if (btns.length) {
                        prev_button = btns.eq(0);
                        next_button = btns.eq(1);
                    }
                }
                if (next_button && prev_button) {
                    next_button.bind('click', function () {
                        if (_this.n == _this.count - 1) {
                            ...
                            _obj.find(".slide_control a").eq(_this.n + 1).triggerHandler('mouseenter');
                        }
                    });
                    prev_button.bind('click', function () {
                        if (_this.n == 0) {
                            _obj.find(".slide_control a").eq(_this.count - 1).triggerHandler('mouseenter');
                        } else {
                            _obj.find(".slide_control a").eq(_this.n - 1).triggerHandler('mouseenter');
                        }
                    });
            ...
            //图片渐影
            var slide = function () {
                _obj.find(".slide_control a").lazyhover(function () {
                    _this.j = $(this).text() - 1;
                    _this.n = _this.j;
                    ...
                    if (_this.first_show) {
                        _this.first_show = 0;
                        $(_obj.find("li:eq(" + _this.j + ")")).show().siblings("li").hide();
                    } else {
                        var li = $(_obj.find("li:eq(" + _this.j + ")"));
                        var next_li;
                        if (_this.count >= _this.j + 1) {
                            next_li = $(_obj.find("li:eq(" + (_this.j + 1) + ")"));
                        }
                        ...
                        //一种是在LI上面控制背景，一种是在A标签上面加背景（POP——V2.5）
                        var _childBg, bg_src = li.attr("lazyload");
                        if (!bg_src) {
                            _childBg = li.find("a");
                            bg_src = _childBg.attr("lazyload");
                        }
                        if (bg_src != undefined) {
                            if (_childBg) {
                                _childBg.css('background', 'url(' + bg_src + ') no-repeat center 0').removeAttr("lazyload");
                            } else {
                                li.css('background-image', 'url(' + bg_src + ')').removeAttr("lazyload");
                            }

                            if (next_li != undefined && next_li.length >= 1) {
                                var _nextChildBg, next_bg_src = next_li.attr("lazyload");
                                if (!next_bg_src) {
                                    _nextChildBg = next_li.find("a");
                                    next_bg_src = _childBg.attr("lazyload");
                                }
                                if (next_bg_src != undefined) {
                                    if (_nextChildBg) {
                                        _nextChildBg.css('background', 'url(' + next_bg_src + ') no-repeat center 0').removeAttr("lazyload");
                                    } else {
                    ...
                    ;
                    resetclass(_obj.find(".slide_control a"), _this.j);
                });
            };

            //滑过停止
            $(_obj[0]).mouseover(function (e) {
                //clearInterval(t);
                if (_this.count > 1 /*&& e.target.tagName.toLowerCase()=='a'*/) {
                    $(_obj.find(".img_slider_btn")).css("display", "block");
                }
            }).mouseout(function (e) {
                //t = setInterval(autoplay,_this.config.time);
                if (_this.count > 1) {
                    $(_obj.find(".img_slider_btn")).hide();
                }
            });
            ...
            var autoplay = function () {
                ...
                $(_obj.find(".slide_control a")).eq(_this.n).triggerHandler('mouseenter');
            };
            //翻页函数
            var resetclass = function (obj, i) {
