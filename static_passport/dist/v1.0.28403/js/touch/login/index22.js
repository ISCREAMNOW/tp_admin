define("touch/login/index", ["../service/sms", "library/q", "../../safeLogin/dialog", "library/template", "templates/Dialog"], function (a) {
    ...
    function c() {
        var a = window.NC_APPKEY, b = window.NC_SCENE, c = [a, (new Date).getTime(), Math.random()].join(":"), d = {
            renderTo: "#aliyun_verify", appkey: a, scene: b, token: c, callback: function (a) {
    ...
    window.NoCaptcha && c(), window.change_code = function () {
        ...
        var a = $(this).parents("form"), c = a.find("#account"), e = a.find("input[type=password]"),
            f = a.find("#signup-hashcode-confirm"), g = "" == c.val(), h = "" == e.val(),
            i = window.has_hash && !window.GT_DATA && "" == f.val(), j = window.GT_VERIFY && !window.GT_DATA,
            k = window.NoCaptcha && !window.ALIYUN_DATA;
        if (g || h) {
            if (g)return new m({
    ...
    var p = $(".register_yzm"), q = $("#dynamic_mobile"), r = $("#dynamic_hash_code"), s = $("#dynamic_password"),
        t = $("#img_dynamic_hash_code"), u = new l({
