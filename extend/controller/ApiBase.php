...
class ApiBase extends Controller{
    ...
    protected $member_id = 0; // 获取用户id 如果会员已经登录 则有值 否则为0
...
//        GlobalService::set('identify',$this->identify);
        ...
        $member_info = UserService::checkToken($token);
        if(!is_int($member_info)){
...
//        $req['code'] = strval($code);
     ...
     */
    protected function response($data = [], $code = 200, $message = '', array $header = ['Access-Control-Allow-Origin' => '*']){
        // TODO 如果参数是整数 不强制转换为字符串
