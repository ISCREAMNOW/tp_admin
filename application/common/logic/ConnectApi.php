...
class ConnectApi extends Controller {
    ...
    public function getStateInfo() {
        $state_array = array();
        $state_array['pc_qq'] = sysconf('qq_isuse');//PC网页端的QQ互联
        $state_array['pc_sn'] = sysconf('sina_isuse');//PC网页端的新浪微博
        $state_array['connect_qq'] = sysconf('app_qq_isuse');//手机客户端的QQ互联
        $state_array['connect_sn'] = sysconf('app_sina_isuse');//手机客户端的新浪微博
        $state_array['connect_wx'] = sysconf('app_weixin_isuse');//手机客户端的微信登录
        $state_array['connect_wap_wx'] = sysconf('wap_weixin_isuse');//wap的微信登录
        $state_array['connect_sms_reg'] = sysconf('sms_register');//手机注册
        $state_array['connect_sms_lgn'] = sysconf('sms_login');//手机登录
        $state_array['connect_sms_psd'] = sysconf('sms_password');//手机短信找回密码
        return $state_array;
    }
    ...
    public function getWxOAuth2Url(){
        $weixin_appid = sysconf('wap_weixin_appid');
    ...
    public function sendCaptcha($phone, $log_type){
        $model_sms_log = D('sms_log');
        $state = true;
        $msg = '手机验证码发送成功';
        $sms_log = $this->ipCaptcha($log_type);
        if(!empty($sms_log) && ($sms_log['add_time'] > time()-DEFAULT_CONNECT_SMS_TIME)) {//同一IP[n]秒内只能发一条短信
            $state = false;
        ...
        if($state && !empty($sms_log) && ($sms_log['add_time'] > time()-DEFAULT_CONNECT_SMS_TIME)) {//同一手机号IP[n]秒内只能发一条短信
            $state = false;
        ...
        if($state == true) {
            ...
            $log_msg = '【'.sysconf('site_name').'】您于'.date("Y-m-d");
            switch ($log_type) {
                case '1':
                    if(sysconf('sms_register') != 1) {
                        $state = false;
                        $msg = '系统没有开启手机注册功能';
                    }
            ...
            if($state == true){
                $sms = new SmsService();
                $result = $sms->smsBao($phone, $log_msg);
    ...
    public function smsRegister($phone, $captcha, $password, $client){
        ...
        if($state == true){
            ...
            $num = substr($phone,-4);
            $member_name = $this->getMemberName('yun', $num);
            $member = array();
    ...
    public function checkSmsCaptcha($phone, $captcha, $log_type){
        ...
        if(empty($sms_log) || ($sms_log['add_time'] < time()-1800)) {//半小时内进行验证为有效
            $state = false;
            $msg = '验证码错误或已过期，重新输入';
        }
        $state_data = array(
    ...
    public function getUserToken($member, $client){
        ...
        $token = md5($member['member_name'] . strval(time()) . strval(rand(0,999999)));
    ...
    public function getWxUserInfo($code, $api_type = ''){
        $weixin_appid = sysconf('weixin_appid');
        $weixin_secret = sysconf('weixin_secret');
        if($api_type == 'api'){
            $weixin_appid = sysconf('app_weixin_appid');
            $weixin_secret = sysconf('app_weixin_secret');
        }
        if($api_type == 'wap'){
            $weixin_appid = sysconf('wap_weixin_appid');
            $weixin_secret = sysconf('wap_weixin_secret');
        }
    ...
    public function getSinaOAuth2Url($sina_type = ''){
        $client_id = sysconf('sina_wb_akey');
        $client_secret = sysconf('sina_wb_skey');
    ...
    public function getSinaUserInfo($code, $client = '', $token = array()){
        ...
        if($client == 'ios' || $client == 'android'){
            $client_id = sysconf('app_sina_akey');
            $client_secret = sysconf('app_sina_skey');
        }
    ...
    public function getQqOAuth2Url($qq_type = ''){
        $client_id = sysconf('qq_appid');
        $client_secret = sysconf('qq_appkey');
    ...
    public function getQqUserInfo($code, $client = '', $token = ''){
        ...
        $client_secret = sysconf('qq_appkey');
        if($client == 'ios' || $client == 'android'){
            $client_id = sysconf('app_qq_akey');
            $client_secret = sysconf('app_qq_skey');
        }
